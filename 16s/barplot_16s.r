...
physeq_factors = psmelt(physeq_16s) %>%
...
##############################################
MpsplS = physeq_16s
sample_data(MpsplS)$Organ_material = "Mean"
...
Spspl = psmelt(Spspl) %>%
	...
	summarise(Abundance = sum(Abundance)) %>%
	group_by(Class) %>%
	mutate(Class = ifelse(max(Abundance) > 0.1, Class, "Other")) %>% # Place classes with less than 0.1% representation among factors in the “other” category
	select(Class, Sample, Abundance)
...
Spspl$Sample = factor(Spspl$Sample, levels = c("Leaf","Stem","Root","Flower","Seed","Mean"))

# Figure 1a. Class level barplot among plant organs (16S)
ggplot(Spspl) +
	...
	geom_text(aes(x = Sample, y = Abundance, 
		...
		size = 10, col = "black") +
	geom_text(aes(x = Sample, y = 102, label=Sum), size = 6) +
...
ggsave("Figure 1a. barplot_Class_Organ_16S.png", width = 15, height = 15)
...
##############################################
MpsplS = physeq_16s
sample_data(MpsplS)$Season = "Mean"
...
Spspl$Sample = factor(Spspl$Sample, levels = c("Autumn","Spring","Mean"))

# Figure 6a. Class level barplot among seasons (16S)
ggplot(Spspl) +
 	geom_col(aes(x = Sample, y = Abundance, fill = Class)) +
	scale_fill_manual(values = Barplot_factors[, "color"]) +
	guides(fill = guide_legend(ncol = 1, byrow = TRUE)) + 
...
ggsave("Figure 6a. barplot_Class_Season_16S.png", width = 14, height = 15)
#
...
Spspl$Sample = factor(Spspl$Sample, levels = c("Pc-1","Pc-2","Pc-3","Pc-4","Pc1-Sakh","Pc2-Sakh","Pc3-Sakh","Mean"))

# Figure 9a. Class level barplot among plants (16S)
ggplot(Spspl) +
 	geom_col(aes(x = Sample, y = Abundance, fill = Class)) +
	scale_fill_manual(values = Barplot_factors[, "color"]) +
	guides(fill = guide_legend(ncol = 1, byrow = TRUE)) + 
	...
	theme(legend.text = element_text(size = 25, colour = "black"),
		legend.title = element_text(size = 18),
		axis.text.x = element_text(size=22, angle=0, vjust = 0.5),
		axis.text.y = element_text(size=25),
		axis.title.y = element_text(size=22)) +
	labs(x = "", y = "Relative abundance, %")

ggsave("Figure 9a. barplot_Class_Plant_16s.png", width = 21, height = 15)
