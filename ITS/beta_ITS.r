...
png("Figure 5d. PCoA plot organs (ITS).png", width = 10, height = 10, res=300, units = "in")
ibd %>%
  ...
  coord_fixed(0.7)+
  theme(legend.text = element_text(size = 23, colour = "black"),
...
png("Figure 8d. PCoA plot seasons (ITS).png", width = 10, height = 10, res=300, units = "in")
ibd %>%
  tax_filter(tax_level = "Genus", min_prevalence = 0.1, verbose = FALSE) %>%
  tax_transform(rank = "Genus", trans = "identity", zero_replace = 0) %>%
  dist_calc(dist = "bray") %>%
...
png("Figure 10d. PCoA plot locations (ITS).png", width = 10, height = 10, res=300, units = "in")
ibd %>%
  tax_filter(tax_level = "Genus", min_prevalence = 0.1, verbose = FALSE) %>%
  tax_transform(rank = "Genus", trans = "identity", zero_replace = 0) %>%
  dist_calc(dist = "bray") %>%
...
write.table(Perm@permanova, sep = "\t", quote = FALSE, file="./Table S8 (ITS).txt")